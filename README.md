...
* **Challenge:** LLMs guess column names from training data (e.g. generating irst_name when the table only has ull_name).
...
| **Database sandbox** | SQLite | Read-only connection, 5s timeout, 100-row etchmany() cap |
...
3. **Execution Sandbox:** The SQLite connection is heavily restricted to a 5-second 	hreading.Timer execution window and a 100-row etchmany() limit to prevent Denial of Service via massive queries.
